...
public class SwitchingSectionsTest extends BaseTest {
    ...
    public void bunsSectionIsActiveOnLoad() {
        ...
        Assert.assertThat("Вкладка 'Булки' не должна быть активной",
                homePage.getClassNameBuns(), not(CoreMatchers.containsString("tab_tab_type_current__2BEPc")));
    ...
    public void switchToSauces() {
        homePage = new HomePage(driver);
        homePage.waitForEnterAccountButton();
        //Assert.assertThat(homePage.getClassNameSauces(), not(CoreMatchers.containsString("tab_tab_type_current__2BEPc")));
        homePage.clickSaucesLink();
        homePage.waitForSaucesActive(10);
    ...
    public void switchToFillings() {
        homePage = new HomePage(driver);
        homePage.waitForEnterAccountButton();
       // Assert.assertThat(homePage.getClassNameFillings(), not(CoreMatchers.containsString("tab_tab_type_current__2BEPc")));
